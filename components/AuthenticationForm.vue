...
const state = reactive({
  formStatus: 'pending', // 'pending' | 'loading'
  error: null,
});
...
const getErrors = (errors = {}) => Object.entries(errors)
  .reduce((messages, [errorMessage, condition]) => {
    if (condition) {
...
const submitForm = async (event) => {
  ...
  if (isFormValid.value && !isLoading.value) {
    ...
    const formData = new FormData(authenticationForm.value);
